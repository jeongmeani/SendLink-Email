...
parser.add_argument('-p','--put_add', help='put the name(initial),e-mail address separated by Comma,ex: kjm,jeongmeani@gmail.com')
...
try:
	...
	if len(format_test)==2:
		...
		print 'put the name(initial),e-mail address separated by Comma,ex: kjm,jeongmeani@gmail.com'
except:
	pass
...
cmd_list='ls * > mail_list.txt'
os.system(cmd_list)
fr=open('mail_list.txt','r')
link=''
link_linux=''
for i in fr.xreadlines():
	i=i.rstrip()
	text='<a href="ftp://id:pw@IP/Download/{0}">{0}</a>'.format(i)  # second {0} is link name, '>{0}' is redirection
	text2='<a href="ftp://id:pw@IP/Download/{0}">ftp://id:pw@IP/Download/{0}</a>'.format(i)
